...
const PUBLIC_DEPLOYMENTS: DeploymentInfo[] = [
  ...
  },
  {
    ...
    chainid: 420,
    notice: `Optimistic Goerli is an internal Optimism development network. You're probably looking for [Optimistic Kovan](../kovan), the public Optimistic Ethereum testnet.`,
  },
]
...
const main = async () => {
  for (const deployment of PUBLIC_DEPLOYMENTS) {
    ...
    if (deployment.explorer) {
      md = addline(md, `- **Block Explorer**: ${deployment.explorer}`)
    }
  ...
  for (const deployment of PUBLIC_DEPLOYMENTS) {
    primary = addline(primary, `- [${deployment.name}](./${deployment.folder})`)
  }
